...
features: `
本アプリでは以下の機能を提供しています。
地図上にピンを表示し、Wi-Fi・電源・声出しOKの条件でリアルタイム絞り込みができます。
また、表示されるスポット情報は現地調査をもとに収集しており、正確性が高い点が特徴です。
各スポットからワンクリックで Google Maps を開けるため、ルート検索や周辺情報を即座に確認できます。
さらに Firebase Hosting と GitHub Actions による CI/CD パイプラインを構築しており、更新が自動デプロイされる設計になっています。
`,
...
-学習記録をクラウド保存
Supabase と連携し、データが自動的にオンライン保存されます。

-アプリ起動時にデータを読み込み表示
保存されている学習内容と学習時間を一覧で確認できます。

-フォームから簡単に記録を追加
学習内容と学習時間を入力して「登録」するだけで一覧に反映されます。

-不要な記録の削除もワンクリック
削除するとクラウド側のデータも同時に消えます。

-合計学習時間を自動計算
記録を追加・削除すると、総時間も自動で更新されます。

-ローディング表示で操作がわかりやすい
データ取得中はスピナーを表示し、処理中であることを明確にします。

-Firebase Hosting にデプロイして公開
実際にブラウザからアクセスできる本番環境を構築。

-Makefile で開発コマンドを整理
make dev や make deploy など、共通操作をワンコマンドに統一。

-GitHub Actions で自動テスト & 自動デプロイ
コードを push すると、テスト → ビルド → 本番デプロイが自動で実行されます。
  `,
...
名刺ページの自動生成
入力した情報が Supabase に保存され、/cards/:id の URL で名刺ページが公開されます。

登録フォーム（名前・紹介文・スキル・SNS）
React Hook Form による入力チェック（必須・英字ID判定など）を実装。

データ管理（Supabase）
名刺データの保存・取得・削除を API 経由で処理。

自動デプロイ（Firebase Hosting）
コードを更新すると GitHub Actions 経由で本番環境に自動反映。

テスト（Jest + React Testing Library）
UI コンポーネントと動作の単体テストを追加。

バッチ処理（GitHub Actions Cron）
毎朝 6:00 に前日のデータを自動削除し、運用データをクリーンに保つ。
  `,
